...
int main()
{
    ...
    const double alpha = 0.5; //Immune waning (compartment flux R moves to S).
    SIRSModel modelDef;

    std::vector<double> init = {0.999, 0.001, 0.0}; //S, I and R initial values
    std::vector<double> params = {beta, sigma, mu, alpha};

    ModelDriver model(&modelDef, params, init);
    model.set_dt(0.005);
    model.set_max_time(1000);
    model.set_output_frequency(10);
    model.run("sirs_example");

    model.export_output();

    std::cout << "\n\nBeta = " << beta << "\tSigma = " << sigma << "\tMu = " << mu << "\tAlpha = " << alpha << "\n";
